...
import { insertBugSchema, insertScanSchema, insertPerformanceMetricSchema, insertAlertSettingsSchema, insertAlertSchema } from "@shared/schema";
...
function validateWebhook(req: any, res: any, next: any) {
  ...
  if (!hmacHeader || !topicHeader || !shopHeader) {
    console.warn('Missing webhook headers:', { hmacHeader, topicHeader, shopHeader });
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/debug/test", (req, res) => {
    res.json({ message: "Debug endpoint is working!", timestamp: new Date().toISOString() });
  });

  // Debug scan scheduler status
  app.get("/api/debug/scheduler", async (req, res) => {
    ...
    console.log('🔍 Debug scheduler - Total scheduled scans:', scheduledScans.length);
    scheduledScans.forEach((scan: any) => {
      console.log(`🔍 Debug scheduler - Store ${scan.storeId}: ${scan.nextScanTime.toISOString()}`);
    });
    
    res.json({
      message: "Scan scheduler status",
      timestamp: new Date().toISOString(),
      status: scanScheduler.getStatus(),
      scheduledScans: scheduledScans.map((scan: any) => ({
        storeId: scan.storeId,
        nextScanTime: scan.nextScanTime.toISOString(),
        timeUntilNext: Math.max(0, scan.nextScanTime.getTime() - Date.now()) / 1000 / 60 // minutes
      }))
    });
  });
  ...
  app.get("/auth", async (req, res) => {
    ...
    try {
      ...
      const authUrl = `https://${shop}/admin/oauth/authorize?` +
        ...
        `scope=${scopes}&` +
        `redirect_uri=${encodeURIComponent(`${serverConfig.isProduction ? 'https://bug-patrol.vercel.app' : 'https://a94717a6db99.ngrok-free.app'}/auth/callback`)}&` +
        `state=${shop}`;
  ...
  try {
    ...
    console.error(`❌ Failed to create app installation record for ${shop}:`, installationError);
  }
  ...
  try {
    ...
    console.error(`❌ Failed to set up default settings for ${shop}:`, settingsError);
  }

      // Create initial performance metrics for new stores
      if (!store.updatedAt || new Date(store.updatedAt).getTime() < Date.now() - 60000) {
      ...
      const redirectUrl = `${serverConfig.isProduction ? 'https://bug-patrol.vercel.app' : 'https://a94717a6db99.ngrok-free.app'}?shop=${shop}&host=${shop}`;
  ...
  async function setupDefaultStoreSettings(storeId: string) {
    try {
      ...
      console.error(`❌ Error setting up default settings for store ${storeId}:`, error);
      throw error;
    }
  }
  ...
  async function registerWebhooks(shop: string, accessToken: string) {
    ...
    const webhookBaseUrl = serverConfig.isProduction 
      ...
      : 'https://a94717a6db99.ngrok-free.app';

    console.log(`Registering webhooks for ${shop} with base URL: ${webhookBaseUrl}`);
    ...
    for (const topic of webhookTopics) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`✅ Webhook registered for ${topic}: ${webhook.webhook.id}`);
        } else {
    ...
    console.log(`Webhook registration complete: ${successCount}/${webhookTopics.length} successful`);
  ...
  */
    ...
    try {
      ...
      if (shop_domain) {
        ...
        if (store) {
          ...
          console.log(`Theme update detected for ${shop_domain}: ${name || `theme_${id}`}`);
        }
      }
    ...
    try {
      ...
      if (shop_domain) {
        ...
        if (store) {
          ...
          console.log(`Theme published for ${shop_domain}: ${name || `theme_${id}`}`);
    ...
    try {
      ...
      if (shop_domain) {
        ...
        if (store) {
          ...
          console.log(`Theme created for ${shop_domain}: ${name || `theme_${id}`}`);
        }
      }
    ...
    try {
      ...
      if (shop_domain) {
        ...
        if (store) {
          ...
          console.log(`Theme deleted for ${shop_domain}: ${name || `theme_${id}`}`);
        }
      }
  ...
  app.get("/api/debug/store/:shop", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed to check store" });
    }
  });

  // Reactivate store endpoint
  app.post("/api/debug/reactivate/:shop", async (req, res) => {
    try {
      const { shop } = req.params;
      const store = await storage.getStoreByDomain(shop);
      ...
      await storage.updateStore(store.id, { isActive: true });
  ...
  app.get("/api/dashboard/:storeId", async (req, res) => {
    try {
      ...
      const [store, bugs, scans, performanceMetrics, alertSettings] = await Promise.all([
      ...
      const criticalBugs = activeBugs.filter(bug => bug.severity === 'critical').length;
      const warningBugs = activeBugs.filter(bug => bug.severity === 'warning').length;
      ...
      if (totalBugs > 0) {
        ...
        const criticalPenalty = Math.min(60, (criticalBugs / Math.max(totalBugs, 1)) * 60); // Max 60% penalty
        const warningPenalty = Math.min(30, (warningBugs / Math.max(totalBugs, 1)) * 30); // Max 30% penalty
      ...
      if (performanceMetrics) {
        ...
        console.log(`🔍 Lighthouse Store Health Calculation for store ${storeId}:`);
        console.log(`  Performance: ${performanceScore} × 0.35 = ${performanceScore * 0.35}`);
        console.log(`  Accessibility: ${accessibilityScore} × 0.15 = ${accessibilityScore * 0.15}`);
        console.log(`  Best Practices: ${bestPracticesScore} × 0.10 = ${bestPracticesScore * 0.10}`);
        console.log(`  SEO: ${seoScore} × 0.20 = ${seoScore * 0.20}`);
        console.log(`  PWA: ${pwaScore} × 0.20 = ${pwaScore * 0.20}`);
        console.log(`  Weighted Total: ${weightedScore} → ${lighthouseHealthScore}`);
      }
      ...
      if (totalBugs > 0) {
        ...
        console.log(`🔍 Final Store Health: ${lighthouseHealthScore} - ${bugPenalty} = ${storeHealth}`);
      } else {
        ...
        const uptime = uptimeData.length > 0 ? 
          (uptimeData.filter(u => u.status === 'up').length / uptimeData.length) * 100 : 100;
        
                        // Get next scan time from scheduler
                const { scanScheduler } = await import('./scan-scheduler');
                const scheduledScans = scanScheduler.getScheduledScans();
                console.log(`🔍 Dashboard API - Store ${storeId}: Found ${scheduledScans.length} scheduled scans`);
                console.log(`🔍 Scheduled scans:`, scheduledScans.map((scan: any) => ({ storeId: scan.storeId, nextScanTime: scan.nextScanTime.toISOString() })));
                
                const storeScheduledScan = scheduledScans.find((scan: any) => scan.storeId === storeId);
                let nextScanTime = storeScheduledScan ? storeScheduledScan.nextScanTime.toISOString() : null;
                
                console.log(`🔍 Store ${storeId} scheduled scan found:`, storeScheduledScan ? 'YES' : 'NO');
                if (storeScheduledScan) {
                  console.log(`🔍 Next scan time for store ${storeId}:`, storeScheduledScan.nextScanTime.toISOString());
                }
        
        // If no scheduled scan found, try to schedule one
        if (!nextScanTime) {
          console.log(`⚠️ No scheduled scan found for store ${storeId}, attempting to schedule...`);
          try {
            ...
            if (!settings?.scanFrequency) {
              console.log(`⚙️ Store ${storeId} has no scan frequency, setting default...`);
              await storage.updateAlertSettings(storeId, { scanFrequency: 60 });
            }
            
            await scanScheduler.scheduleStoreScan(storeId);
            const updatedScheduledScans = scanScheduler.getScheduledScans();
            const updatedStoreScan = updatedScheduledScans.find((scan: any) => scan.storeId === storeId);
            nextScanTime = updatedStoreScan ? updatedStoreScan.nextScanTime.toISOString() : null;
            console.log(`✅ Successfully scheduled scan for store ${storeId}:`, nextScanTime);
          } catch (error) {
            ...
            console.log(`🔄 Using fallback scan time for store ${storeId}:`, nextScanTime);
          }
        }
        ...
        if (performanceMetrics) {
          ...
          pageSpeed = performanceMetrics.lcp ? parseFloat(performanceMetrics.lcp) : null;
          ...
          if (allMetrics.length > 1) {
            ...
            const previousLcp = allMetrics[1]?.lcp ? parseFloat(allMetrics[1].lcp) : null;
            ...
            const currentFcp = performanceMetrics.fcp ? parseFloat(performanceMetrics.fcp) : null;
            const previousFcp = allMetrics[1]?.fcp ? parseFloat(allMetrics[1].fcp) : null;
            const currentCls = performanceMetrics.cls ? parseFloat(performanceMetrics.cls) : null;
            const previousCls = allMetrics[1]?.cls ? parseFloat(allMetrics[1].cls) : null;
            const currentScore = performanceMetrics.overallScore;
            const previousScore = allMetrics[1]?.overallScore;
            
            trends = {
              lcp: currentLcp && previousLcp ? { trend: currentLcp < previousLcp ? 'improving' : 'worsening' } : undefined,
              fcp: currentFcp && previousFcp ? { trend: currentFcp < previousFcp ? 'improving' : 'worsening' } : undefined,
              cls: currentCls && previousCls ? { trend: currentCls < previousCls ? 'improving' : 'worsening' } : undefined,
              overallScore: currentScore && previousScore ? { trend: currentScore > previousScore ? 'improving' : 'worsening' } : undefined
            };
          }
        }
  ...
  app.get("/api/performance/:storeId", async (req, res) => {
    try {
      ...
      // Get store and latest performance metrics
      ...
      const recentMetrics = allMetrics.filter(metric => 
        metric.measuredAt && metric.measuredAt !== null && new Date(metric.measuredAt) >= thirtyDaysAgo
      );

      // Calculate 30-day averages
      const validRecentMetrics = recentMetrics.filter(metric => 
        metric.lcp && metric.lcp !== 'NaN' && metric.lcp !== 'null' && 
        metric.fcp && metric.fcp !== 'NaN' && metric.fcp !== 'null' && 
        metric.cls && metric.cls !== 'NaN' && metric.cls !== 'null'
      );

      console.log(`Found ${validRecentMetrics.length} valid metrics out of ${recentMetrics.length} recent metrics`);

      const thirtyDayAverages = validRecentMetrics.length > 0 ? {
        lcp: validRecentMetrics.reduce((sum, m) => sum + parseFloat(m.lcp || '0'), 0) / validRecentMetrics.length,
        fcp: validRecentMetrics.reduce((sum, m) => sum + parseFloat(m.fcp || '0'), 0) / validRecentMetrics.length,
        cls: validRecentMetrics.reduce((sum, m) => sum + parseFloat(m.cls || '0'), 0) / validRecentMetrics.length,
        overallScore: validRecentMetrics.reduce((sum, m) => sum + (m.overallScore || 0), 0) / validRecentMetrics.length,
      ...
      res.json({
        ...
        latestScanInfo: latestMetrics && latestMetrics.measuredAt ? {
          scanTime: latestMetrics.measuredAt,
          scanAge: Math.floor((Date.now() - new Date(latestMetrics.measuredAt).getTime()) / (1000 * 60 * 60)), // hours ago
          isRecent: (Date.now() - new Date(latestMetrics.measuredAt).getTime()) < (1000 * 60 * 60 * 24) // within 24 hours
        } : null,
        thirtyDayAverages,
        historicalMetrics: recentMetrics,
        trends: {
          lcp: recentMetrics.length > 1 ? {
            average: recentMetrics.reduce((sum, m) => sum + parseFloat(m.lcp || '0'), 0) / recentMetrics.length,
            trend: recentMetrics[recentMetrics.length - 1]?.lcp && recentMetrics[0]?.lcp && 
                   parseFloat(recentMetrics[recentMetrics.length - 1].lcp) > parseFloat(recentMetrics[0].lcp) ? 'worsening' : 'improving'
          } : null,
          fcp: recentMetrics.length > 1 ? {
            average: recentMetrics.reduce((sum, m) => sum + parseFloat(m.fcp || '0'), 0) / recentMetrics.length,
            trend: recentMetrics[recentMetrics.length - 1]?.fcp && recentMetrics[0]?.fcp && 
                   parseFloat(recentMetrics[recentMetrics.length - 1].fcp) > parseFloat(recentMetrics[0].fcp) ? 'worsening' : 'improving'
          } : null,
          cls: recentMetrics.length > 1 ? {
            average: recentMetrics.reduce((sum, m) => sum + parseFloat(m.cls || '0'), 0) / recentMetrics.length,
            trend: recentMetrics[recentMetrics.length - 1]?.cls && recentMetrics[0]?.cls && 
                   parseFloat(recentMetrics[recentMetrics.length - 1].cls) > parseFloat(recentMetrics[0].cls) ? 'worsening' : 'improving'
          } : null,
          overallScore: recentMetrics.length > 1 ? {
            average: recentMetrics.reduce((sum, m) => sum + (m.overallScore || 0), 0) / recentMetrics.length,
            trend: recentMetrics[recentMetrics.length - 1]?.overallScore && recentMetrics[0]?.overallScore && 
                   recentMetrics[recentMetrics.length - 1].overallScore < recentMetrics[0].overallScore ? 'worsening' : 'improving'
          } : null
        }
      });
    } catch (error) {
  ...
  // Debug endpoint to check all bugs
  app.get("/api/debug/bugs", async (req, res) => {
    try {
      const allBugs = await storage.getAllBugs();
      res.json({ 
        totalBugs: allBugs.length,
        bugs: allBugs.slice(0, 5) // Show first 5 bugs
      });
    } catch (error) {
  ...
  // Debug endpoint to check image details in bugs
  app.get("/api/debug/image-bugs", async (req, res) => {
    try {
      const allBugs = await storage.getAllBugs();
      const accessibilityBugs = allBugs.filter(bug => bug.type === 'accessibility');
      const bugsWithImageDetails = accessibilityBugs.filter(bug => 
        bug.details && typeof bug.details === 'object' && 'imageDetails' in bug.details && Array.isArray((bug.details as any).imageDetails)
      );
      
      res.json({ 
        totalAccessibilityBugs: accessibilityBugs.length,
        bugsWithImageDetails: bugsWithImageDetails.length,
        sampleBug: bugsWithImageDetails[0] || null,
        allAccessibilityBugs: accessibilityBugs.map(bug => ({
          id: bug.id,
          title: bug.title,
          hasImageDetails: !!(bug.details && typeof bug.details === 'object' && 'imageDetails' in bug.details),
          imageDetailsCount: Array.isArray((bug.details as any)?.imageDetails) ? (bug.details as any).imageDetails.length : 0
        }))
      });
    } catch (error) {
      console.error('Error fetching image bugs:', error);
      res.status(500).json({ message: "Failed to fetch image bugs" });
    }
  });
  ...
  app.get("/api/debug/performance", async (req, res) => {
    try {
      ...
      const metricsByStore = allStores.map(store => {
        const storeMetrics = allMetrics.filter(metric => metric.storeId === store.id);
        const latestMetric = storeMetrics[0]; // Already sorted by date desc
        
        return {
          storeId: store.id,
          storeDomain: store.shopifyDomain,
          totalMetrics: storeMetrics.length,
          latestMetric: latestMetric ? {
            id: latestMetric.id,
            lcp: latestMetric.lcp,
            fcp: latestMetric.fcp,
            cls: latestMetric.cls,
  ...
  app.get("/api/bugs/:storeId", async (req, res) => {
    try {
      ...
      let filteredBugs = allBugs.filter(bug => {
        ...
        if (severityFilter && severityFilter !== 'all' && severityFilter !== '' && bug.severity !== severityFilter) {
          return false;
        }
        
        // Status filter
        const statusFilter = status as string;
        if (statusFilter && statusFilter !== 'all' && statusFilter !== '' && bug.status !== statusFilter) {
          return false;
        }
        
        // Type filter
        const typeFilter = type as string;
        if (typeFilter && typeFilter !== 'all' && typeFilter !== '' && bug.type !== typeFilter) {
          return false;
        }
      ...
      console.log('Filter values:', { search, severity, status, type, sortBy, sortOrder });
      ...
      const severities = Array.from(new Set(allBugs.map(bug => bug.severity)));
      // TODO: Uncomment in next version: const statuses = Array.from(new Set(allBugs.map(bug => bug.status)));
      const statuses = Array.from(new Set(allBugs.map(bug => bug.status).filter(status => status !== 'resolved')));
  ...
  app.get("/api/bugs/:storeId/:bugId", async (req, res) => {
    try {
      ...
      const relatedBugs = allBugs
        .filter(b => b.id !== bugId && (b.type === bug.type || b.severity === bug.severity))
        .slice(0, 5);
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            console.log(`Basic scan ${scan.id} completed for store ${scanData.storeId}`);
          } catch (error) {
  ...
  app.get("/api/scans/:scanId", async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: "Failed to get scan status", error: String(error) });
    }
  });
  ...
  app.patch("/api/alert-settings/:storeId", async (req, res) => {
    try {
      ...
      if (settingsData.scanFrequency !== undefined) {
        console.log(`📅 Scan frequency updated for store ${storeId}: ${settingsData.scanFrequency} minutes`);
        const { scanScheduler } = await import("./scan-scheduler");
  ...
  app.get("/api/test-mode", async (req, res) => {
    try {
      ...
      res.json({
        testMode: serverConfig.testMode,
        testWebsite: serverConfig.testWebsite
      });
    } catch (error) {
      console.error("Error fetching test mode status:", error);
      res.status(500).json({ message: "Failed to fetch test mode status" });
    }
  });
  ...
  app.get("/api/uptime/:storeId", async (req, res) => {
    try {
      ...
      const upChecks = last24Hours.filter(check => check.status === 'up').length;
      const uptimePercentage = totalChecks > 0 ? (upChecks / totalChecks) * 100 : 100;

      // Calculate average response time
      const avgResponseTime = last24Hours.length > 0 
        ? Math.round(last24Hours.reduce((sum, check) => sum + (check.responseTime || 0), 0) / last24Hours.length)
        : 0;
      ...
      const incidentsThisMonth = recentChecks.filter(check => 
        check.status === 'down' && check.checkedAt && new Date(check.checkedAt) >= thisMonth
      ).length;

      // Find last downtime
      const lastDowntime = recentChecks.find(check => check.status === 'down');
      const lastDowntimeText = lastDowntime && lastDowntime.checkedAt
      ...
      res.json({
        ...
        downtimeDuration: lastDowntime?.downUntil && lastDowntime?.downFrom
          ? `${Math.round((new Date(lastDowntime.downUntil).getTime() - new Date(lastDowntime.downFrom).getTime()) / (1000 * 60))} minutes`
          : 'Unknown',
  ...
  app.get("/api/app-installations/:storeId", async (req, res) => {
    try {
      ...
      const appInstallationsData = await storage.getAppInstallationsByStoreId(storeId);
  ...
  try {
    ...
    if (!settings || !settings.emailEnabled) {
      return res.status(400).json({ message: "Email notifications not enabled" });
    }
  ...
  app.post("/api/test-slack/:storeId", async (req, res) => {
    try {
      ...
      if (!settings || !settings.slackEnabled) {
        return res.status(400).json({ message: "Slack notifications not enabled" });
      }
      ...
      try {
        ...
        res.status(500).json({
          success: false,
          message: "Failed to send test Slack message. Please check your bot token and channel ID."
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (hasSlackEnabled) {
        try {
          ...
          results.slack = true;
          console.log(`Test Slack notification sent successfully for store ${storeId}`);
        } catch (slackError) {
          console.error("Failed to send Slack test alert:", slackError);
        }
      } else {
        console.log(`Slack test skipped for store ${storeId} - not enabled or missing credentials`);
      }
      ...
      if (hasEmailEnabled) {
        try {
          ...
          console.log(`Test email notification sent successfully for store ${storeId}`);
        } catch (emailError) {
          console.error("Failed to send email test alert:", emailError);
        }
      } else {
        console.log(`Email test skipped for store ${storeId} - not enabled or missing email address`);
      }
  ...
  app.get("/api/admin/overview", async (req, res) => {
    try {
      ...
      const totalRevenue = stores.reduce((sum, store) => {
        const planRevenue = store.plan === 'enterprise' ? 199 : store.plan === 'pro' ? 89 : 29;
        return sum + planRevenue;
      }, 0);
      const totalScans = allScans.length;
      const criticalIssues = allBugs.filter(bug => bug.severity === 'critical').length;
      const averageUptime = stores.length > 0 ? 
        stores.reduce((sum, store) => sum + (store.uptime || 99.9), 0) / stores.length : 99.9;
    ...
    try {
      ...
      const adminStores = stores.map(store => ({
        ...
        lastScan: store.lastScanAt ? 
          `${Math.floor((Date.now() - new Date(store.lastScanAt).getTime()) / (1000 * 60))} minutes ago` : 
          'Never',
        status: store.status || 'active',
        uptime: `${store.uptime || 99.9}%`,
        totalScans: store.totalScans || 0,
        criticalIssues: store.criticalIssues || 0,
        revenue: `$${store.plan === 'enterprise' ? 199 : store.plan === 'pro' ? 89 : 29}/mo`
      }));
  ...
  app.get("/api/admin/stores/:storeId", async (req, res) => {
    try {
      ...
      const performanceMetrics = await storage.getPerformanceMetricsByStore(storeId);
      ...
      const storeDetails = {
        ...
        subscription: {
          plan: store.plan || 'basic',
          status: store.status || 'active',
          amount: store.plan === 'enterprise' ? 199 : store.plan === 'pro' ? 89 : 29,
          nextBilling: store.nextBillingDate || '2024-07-15',
          features: store.plan === 'enterprise' ? 
            ["Unlimited Scans", "24/7 Monitoring", "Priority Support", "Custom Reports"] :
            store.plan === 'pro' ? 
            ["Advanced Scans", "Daily Monitoring", "Email Support", "Basic Reports"] :
        ...
        recentActivity: scans.slice(0, 5).map(scan => ({
          ...
          timestamp: scan.completedAt ? 
            `${Math.floor((Date.now() - new Date(scan.completedAt).getTime()) / (1000 * 60 * 60))} hours ago` : 
            'In progress',
          status: scan.status
        })),
        technicalInfo: {
          ...
          lastUpdated: store.updatedAt?.toISOString().split('T')[0] || 'Unknown'
        }
      };
  ...
  app.get("/api/admin/analytics", async (req, res) => {
    try {
      ...
      const overview = {
        ...
        avgIssuesPerStore: stores.length > 0 ? Math.round((allBugs.length / stores.length) * 10) / 10 : 0,
      ...
      const planDistribution = [
        {
          plan: "Enterprise",
          customers: stores.filter(store => store.plan === 'enterprise').length,
          revenue: stores.filter(store => store.plan === 'enterprise').length * 199,
          percentage: stores.length > 0 ? Math.round((stores.filter(store => store.plan === 'enterprise').length / stores.length) * 1000) / 10 : 0
        },
        {
          plan: "Pro",
          customers: stores.filter(store => store.plan === 'pro').length,
          revenue: stores.filter(store => store.plan === 'pro').length * 89,
          percentage: stores.length > 0 ? Math.round((stores.filter(store => store.plan === 'pro').length / stores.length) * 1000) / 10 : 0
        },
        {
          plan: "Basic",
          customers: stores.filter(store => store.plan === 'basic').length,
          revenue: stores.filter(store => store.plan === 'basic').length * 29,
          percentage: stores.length > 0 ? Math.round((stores.filter(store => store.plan === 'basic').length / stores.length) * 1000) / 10 : 0
        }
      ];

      const topIssues = [
        { type: "Broken Links", count: allBugs.filter(bug => bug.type === 'broken_link').length, stores: new Set(allBugs.filter(bug => bug.type === 'broken_link').map(bug => bug.storeId)).size },
        { type: "Performance", count: allBugs.filter(bug => bug.type === 'performance').length, stores: new Set(allBugs.filter(bug => bug.type === 'performance').map(bug => bug.storeId)).size },
        { type: "JavaScript Errors", count: allBugs.filter(bug => bug.type === 'javascript_error').length, stores: new Set(allBugs.filter(bug => bug.type === 'javascript_error').map(bug => bug.storeId)).size },
        { type: "Missing Images", count: allBugs.filter(bug => bug.type === 'missing_image').length, stores: new Set(allBugs.filter(bug => bug.type === 'missing_image').map(bug => bug.storeId)).size },
        { type: "SEO Issues", count: allBugs.filter(bug => bug.type === 'seo').length, stores: new Set(allBugs.filter(bug => bug.type === 'seo').map(bug => bug.storeId)).size }
